...
while ready_queue or block_queue:
    if ready_queue:
        ...
        IR = current_process["process_to_be_executed"][
            : min(current_process["execution_time"], quantum_time)
        ]
        PC = min(current_process["execution_time"], quantum_time)

        needs_resource = random.choice([True, False])
        if needs_resource:
            ...
            elements_needing_resources[current_process_name] = current_process[
            ...
            current_process["process_to_be_executed"] = current_process[
                "process_to_be_executed"
            ][len(IR) :]
        ...
        if current_process["resource_needed"]:
            print(
                f"{current_process_name} needs a resource for element {element_needing_resource}."
            )
