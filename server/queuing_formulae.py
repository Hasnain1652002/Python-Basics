...
def calculate_prob_zero_customers(arrival_rate, service_rate, num_of_servers):
    ...
    denominator = sum([(rho**n) / math.factorial(n) for n in range(num_of_servers)])
...
def cal_avg_queue_length_GG1(
    ...
    rho = arrival_rate / service_rate
    Ca_squared = arrival_time_var * arrival_rate**2
    Cs_squared = service_time_var * service_rate**2

    return (rho**2 * (1 + Cs_squared) * (Ca_squared + rho**2 * Cs_squared)) / (
        2 * (1 - rho) * (1 + rho**2 * Cs_squared)
    )
...
def cal_avg_queue_length_GGC(
    ...
    avg_queue_waiting_time_GGC = (
        avg_queue_waiting_time_MMC * (Cs_squared + Ca_squared) / 2
    )
...
def cal_avg_queue_length_MGC(
    ...
    return cal_avg_queue_length_GGC(
        arrival_rate, 1 / arrival_rate, service_rate, service_time_var, num_of_servers
    )
